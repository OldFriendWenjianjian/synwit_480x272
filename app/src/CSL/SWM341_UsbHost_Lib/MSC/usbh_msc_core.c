...
USBH_Status USBH_MSC_Process(USBH_Info_t *phost)
{
	...
	case USBH_MSC_GET_MAXLUN:
		...
		{
			/* after clear STALL, enter USBH_MSC_TEST_UNIT_READY */
	...
	case USBH_MSC_CLEAR_STALL:
		...
		if(status == USBH_OK)
		{
			USBH_MSC_Info.MaxLUN = 0;	// unsupport GetMaxLUN request, assume that there is only one logical unit
...
/*******************************************************************************************************************************
* @brief	USB host MSC error handle
* @param
* @return
*******************************************************************************************************************************/
