    ...
    case USBH_CDC_SEND_WAIT:
		...
		if(resp != USBR_NO)
		{
			if(resp == USBR_ACK)
			{
				...
				if(USBH_CDC_Info.TxRdPtr == USBH_CDC_Info.TxWrPtr)	// data in USBH_CDC_Info.TxBuff all sent
				{
					USBH_CDC_Info.TxRdPtr = 0;
					USBH_CDC_Info.TxWrPtr = 0;
				}
...
/*******************************************************************************************************************************
* @brief	USB host CDC send
* @param
* @return
*******************************************************************************************************************************/
uint32_t USBH_CDC_Send(uint8_t *data, uint16_t size)
{
	uint16_t space = USBH_CDC_BUFF_SIZE - USBH_CDC_Info.TxWrPtr;	// Number of data that can be written
...
uint32_t USBH_CDC_Read(uint8_t *buff, uint16_t size)
{
	...
	if(size)
	{
		...
		if(USBH_CDC_Info.RxRdPtr == USBH_CDC_Info.RxWrPtr)	// all data in USBH_CDC_Info.RxBuff has been read out
		{
			USBH_CDC_Info.RxRdPtr = 0;
			USBH_CDC_Info.RxWrPtr = 0;
		}
