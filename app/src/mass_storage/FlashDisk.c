...
static uint8_t FLASH_Cache_Dirty = 0;
...
{	
    ...
    if(FLASH_Cache_Dirty)
    {
        ...
        #define SFC_WRITE_FUNC      SFC_GPIOWrite //SFC_Write //SFC_GPIOWrite
        
#if SFC_WRITE_FUNC == SFC_GPIOWrite
		...
		for(int i = 0; i < FLASH_BLOCK_SIZE; i += 256) {
			SFC_WRITE_FUNC(DATA_FLASH_BASE + Flash_Block_Addr + i, (uint32_t *)&FLASH_Block_Cache[i], 256/4);
        }
        
#if SFC_WRITE_FUNC == SFC_GPIOWrite /* SFC使用专用的FSPI（Flash SPI）接口连接SPI Flash */
        __DSB(); __ISB();
