...
static inline uint32_t lv_color_to32_bgr(lv_color_t color)
{
    uint32_t bgr = lv_color_to32(color);
    uint8_t c0 = (uint8_t)(bgr & 0x000000ff);
    uint8_t c16 = (uint8_t)((bgr >> 16) & 0x000000ff);
    bgr &= 0xff00ff00;
    bgr |= (c0 << 16) + (c16);
...
static inline lv_color_t lv_color_bgr(uint32_t c)
{
    return lv_color_make((uint8_t)(c & 0xFF), (uint8_t)((c >> 8) & 0xFF), (uint8_t)((c >> 16) & 0xFF));
}
...
//! @cond Doxygen_Suppress
//!
LV_EXPORT_API LV_ATTRIBUTE_FAST_MEM void lv_color_fill(lv_color_t * buf, lv_color_t color, uint32_t px_num);

//! @endcond
LV_EXPORT_API lv_color_t lv_color_lighten(lv_color_t c, lv_opa_t lvl);

LV_EXPORT_API lv_color_t lv_color_darken(lv_color_t c, lv_opa_t lvl);
 ...
 */
LV_EXPORT_API lv_color_t lv_color_hsv_to_rgb(uint16_t h, uint8_t s, uint8_t v);
 ...
 */
LV_EXPORT_API lv_color_hsv_t lv_color_rgb_to_hsv(uint8_t r8, uint8_t g8, uint8_t b8);
 ...
 */
LV_EXPORT_API lv_color_hsv_t lv_color_to_hsv(lv_color_t color);
