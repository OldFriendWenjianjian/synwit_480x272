 ...
 **********************/
    ...
    static void * ent_alloc(lv_mem_ent_t * e, size_t size, size_t addr_align);
...
DEFRAG_RETRY:
    ...
    do {
        ...
        if (e != NULL) {
            alloc = ent_alloc(e, size, addr_alignment);
        }
        /* End if there is not next entry OR the alloc. is successful*/
    } while (e != NULL && alloc == NULL);

    if (defrag_retried == false && alloc == NULL) {
        defrag_retried = true;
        lv_mem_defrag();
        goto DEFRAG_RETRY;
    }
    ...
    if (alloc == NULL) {
        ...
        do {
            /* Get the next entry*/
            e = ent_get_next(e);
            if (e != NULL && e->header.s.used == 0) {
                free_size += e->header.s.d_size;
            }
            /* End if there is not next entry OR the alloc. is successful*/
        } while (e != NULL);

        if ((LV_MEM_SIZE - free_size) > mem_max_size) {
            mem_max_size = LV_MEM_SIZE - free_size;
        }
    #else
 ...
 */
static void * ent_alloc(lv_mem_ent_t * e, size_t size, size_t addr_alignment)
{
    void * alloc = NULL;
    /*If the memory is free and big enough then use it */
    if(e->header.s.used == 0 && e->header.s.d_size >= size) {
        addr_alignment = (addr_alignment + ALIGN_MASK) & (~ALIGN_MASK);
        uint32_t addr_align_mask = addr_alignment - 1;
        uint32_t addr32 = (uint32_t)&e->first_data;
        if (addr_alignment == 0 || (addr32 & ~addr_align_mask) == addr32) {
            ...
            uint8_t* align_addr = (uint8_t *)((addr32 + addr_align_mask) & ~addr_align_mask);
            uint32_t frag_size = align_addr - &e->first_data - sizeof(lv_mem_header_t);
            if (frag_size == 0) {
                /* No space for lv_mem_header_t in this situation, move
                 * align_addr to the next align position.
                 */
                align_addr += addr_alignment;
                frag_size += addr_alignment;
            }

            if (e->header.s.d_size >= (frag_size + size + sizeof(lv_mem_header_t))) {
                ent_trunc(e, frag_size);
                e->header.s.used = 0;

                uint8_t* e_data = &e->first_data;
                lv_mem_ent_t* new_ent = (lv_mem_ent_t*)&e_data[e->header.s.d_size];
                ent_trunc(new_ent, size);
                new_ent->header.s.used = 1;
                alloc = &new_ent->first_data;
            }
        }
    }
