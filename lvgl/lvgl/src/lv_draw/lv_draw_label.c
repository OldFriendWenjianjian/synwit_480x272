                              ...
                              const uint8_t * map_p, lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    uint8_t txt_rgb[3] = {(uint8_t)color.ch.red, (uint8_t)color.ch.green, (uint8_t)color.ch.blue};
#else
...
#if LV_COLOR_16_SWAP == 0
                uint8_t bg_rgb[3] = {(uint8_t)vdb_buf_tmp->ch.red, (uint8_t)vdb_buf_tmp->ch.green, (uint8_t)vdb_buf_tmp->ch.blue};
#else
