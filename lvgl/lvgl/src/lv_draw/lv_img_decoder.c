...
#define CF_BUILT_IN_LAST LV_IMG_CF_GRAYSCALE_8BIT
 ...
 */
lv_res_t lv_img_decoder_open(lv_img_decoder_dsc_t * dsc, const void * src, lv_color_t color, lv_img_decode_mode_t decode_mode)
{
    _lv_memset_00(dsc, sizeof(lv_img_decoder_dsc_t));

    dsc->color    = color;
    dsc->src_type = lv_img_src_get_type(src);
    dsc->mode = decode_mode;
...
lv_res_t lv_img_decoder_uncompress(lv_img_decoder_dsc_t* dsc, lv_color_t* optional_output_buffer, uint32_t stride)
{
    lv_res_t res = LV_RES_INV;
    if (dsc->decoder->uncompress_cb) res = dsc->decoder->uncompress_cb(dsc->decoder, dsc, optional_output_buffer, stride);
...
void lv_img_decoder_set_uncompress_cb(lv_img_decoder_t* decoder, lv_img_decoder_uncompress_f_t uncompress_cb)
{
    decoder->uncompress_cb = uncompress_cb;
}
...
lv_res_t lv_img_decoder_built_in_open(lv_img_decoder_t * decoder, lv_img_decoder_dsc_t * dsc)
{
    ...
    if(cf == LV_IMG_CF_TRUE_COLOR || cf == LV_IMG_CF_TRUE_COLOR_ALPHA || 
        cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED || cf == LV_IMG_CF_GRAYSCALE_8BIT) {
        if(dsc->src_type == LV_IMG_SRC_VARIABLE) {
            /* In case of uncompressed formats the image stored in the ROM/RAM.
             * So simply give its pointer*/
            dsc->img_data = ((lv_img_dsc_t *)dsc->src)->data;
            dsc->stride = ((lv_img_dsc_t*)dsc->src)->stride;
            return LV_RES_OK;
        }
        else {
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA ||
       dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
        ...
        if(dsc->src_type == LV_IMG_SRC_FILE) {
            ...
            dsc->header.cf == LV_IMG_CF_GRAYSCALE_8BIT) {
...
#error "Invalid LV_COLOR_DEPTH. Check it in lv_conf.h"
    ...
    if (dsc->header.cf == LV_IMG_CF_GRAYSCALE_8BIT) {
        for (i = 0; i < len; i++) {
            *buf++ = *data_tmp++;
        }
    }
