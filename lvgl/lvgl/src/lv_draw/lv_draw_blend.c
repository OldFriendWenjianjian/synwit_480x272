...
#include "../lv_core/lv_refr.h"
    ...
    #include "../lv_gpu/lv_gpu_swm341_dma2d.h"
#elif LV_USE_GPU_SWM330_DMA2D
    #include "../lv_gpu/lv_gpu_swm330_dma2d.h"
#endif

/*********************
 *      DEFINES
 *********************/
#define GPU_SIZE_LIMIT      100 //240
...
#if (LV_USE_GPU || LV_USE_GPU_STM32_DMA2D || LV_USE_GPU_SWM341_DMA2D) && (LV_USE_GPU_NXP_PXP == 0) && (LV_USE_GPU_NXP_VG_LITE == 0)
    LV_ATTRIBUTE_DMA static lv_color_t blend_buf[LV_HOR_RES_MAX];
#endif

#if LV_USE_GPU_SWM330_DMA2D && !(LV_COLOR_SCREEN_TRANSP && LV_COLOR_DEPTH != 32)
    LV_ATTRIBUTE_DMA static lv_color_t blend_buf[LV_HOR_RES_MAX]; /* DMA2D: blend mode no support register_color to memory with opa, need create memory and fill color */
    LV_ATTRIBUTE_DMA static uint16_t alpha_buf[LV_HOR_RES_MAX]; /* DMA2D: extern alpha blend mode opa_unit fix is 16bit, need convert */
#endif
...
lv_color_t* _lv_disp_buf_projection_first(lv_disp_t* disp, lv_color_t* disp_buf,
                                    const lv_area_t* disp_area, const lv_area_t* draw_area,
                                    int32_t* disp_width)
{
    lv_coord_t projx, projy;
    
    if (disp->driver.sw_rotate == 0) {
        if (lv_disp_is_true_double_buf(disp)) {
            projy = disp_area->y1 + draw_area->y1;
            projx = disp_area->x1 + draw_area->x1;
            *disp_width = lv_disp_get_hor_res(disp);
        }
        else {
            /* For display mode 2 (2 small alternative buffers + 1 frame buffer)*/
            projy = disp_area->y1 + (draw_area->y1 - disp_area->y1);
            projx = disp_area->x1 + (draw_area->x1 - disp_area->x1);
            *disp_width = lv_area_get_width(disp_area);
        }
    ...
    return disp_buf + (*disp_width) * projy + projx;
}
                                              ...
                                              const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    int32_t disp_w;
    lv_color_t* disp_buf_first = _lv_disp_buf_projection_first(
                                _lv_refr_get_disp_refreshing(),
                                disp_buf, disp_area, draw_area, &disp_w);
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
...
#elif LV_USE_GPU_SWM341_DMA2D
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_gpu_swm341_dma2d_fill(disp_buf_first, disp_w, color, draw_area_w, draw_area_h);
                return;
            }
#elif LV_USE_GPU_SWM330_DMA2D
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_gpu_swm330_dma2d_fill(disp_buf_first, disp_w, color, draw_area_w, draw_area_h);
                return;
            }
#elif LV_USE_GPU
        ...
        /*No mask with opacity*/
            ...
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                ...
                lv_coord_t line_h = LV_HOR_RES_MAX / draw_area_w;
                for(y = 0; y <= draw_area_h - line_h; y += line_h) {
                    lv_gpu_swm341_dma2d_blend(disp_buf_first, disp_w, blend_buf, opa, draw_area_w, draw_area_w, line_h);
                    lv_gpu_swm341_dma2d_wait_cb(NULL);
                    disp_buf_first += disp_w * line_h;
                }

                if(y != draw_area_h) {
                    lv_gpu_swm341_dma2d_blend(disp_buf_first, disp_w, blend_buf, opa, draw_area_w, draw_area_w, draw_area_h - y);
                }

                return;
            }
#elif LV_USE_GPU_SWM330_DMA2D && !(LV_COLOR_SCREEN_TRANSP && LV_COLOR_DEPTH != 32)
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_coord_t line_h = LV_HOR_RES_MAX / draw_area_w;
                if (line_h > draw_area_h) {
                    line_h = draw_area_h;
                }

                //if(blend_buf[line_h * draw_area_w - 1].full != color.full) {
                    lv_gpu_swm330_dma2d_fill(blend_buf, draw_area_w, color, draw_area_w, line_h);
                //}

                for(y = 0; y <= draw_area_h - line_h; y += line_h) {
                    lv_gpu_swm330_dma2d_blend(disp_buf_first, disp_w, blend_buf, draw_area_w, opa, draw_area_w, line_h);
                    disp_buf_first += disp_w * line_h;
                }

                if(y != draw_area_h) {
                    lv_gpu_swm330_dma2d_blend(disp_buf_first, disp_w, blend_buf, draw_area_w, opa, draw_area_w, draw_area_h - y);
                }
...
#elif LV_USE_GPU_SWM341_DMA2D && 0
        if(lv_area_get_size(draw_area) > GPU_SIZE_LIMIT) {
            lv_gpu_swm341_dma2d_fill_mask(disp_buf_first, disp_w, color, mask, opa, draw_area_w, draw_area_h);
            return;
        }
#elif LV_USE_GPU_SWM330_DMA2D && !(LV_COLOR_SCREEN_TRANSP && LV_COLOR_DEPTH != 32)
        if(lv_area_get_size(draw_area) > GPU_SIZE_LIMIT) {
            ...
            for(y = 0; y <= draw_area_h - line_h; y += line_h) {
                for (lv_coord_t i = 0; i < line_h; ++i) {
                    for (lv_coord_t x = 0; x < draw_area_w; ++x) {
                        lv_opa_t last_mask = mask[i * draw_area_w + x];
                        alpha_buf[i * draw_area_w + x] = (uint32_t)((uint32_t)(last_mask) * opa) >> 8;
                        //alpha_buf[i * draw_area_w + x] = (opa > LV_OPA_MAX) ? last_mask : (last_mask == LV_OPA_COVER ? opa : (uint32_t)((uint32_t)(last_mask) * opa) >> 8);
                    }
                }
                lv_gpu_swm330_dma2d_blend_mask(disp_buf_first, disp_w, blend_buf, draw_area_w, (const lv_opa_t *)alpha_buf, draw_area_w, line_h);

                disp_buf_first += disp_w * line_h;
                mask += draw_area_w * line_h;
            }

            if(y != draw_area_h) {
                for (lv_coord_t i = 0; i < draw_area_h - y; ++i) {
                    for (lv_coord_t x = 0; x < draw_area_w; ++x) {
                        lv_opa_t last_mask = mask[i * draw_area_w + x];
                        alpha_buf[i * draw_area_w + x] = (uint32_t)((uint32_t)(last_mask) * opa) >> 8;
                        //alpha_buf[i * draw_area_w + x] = (opa > LV_OPA_MAX) ? last_mask : (last_mask == LV_OPA_COVER ? opa : (uint32_t)((uint32_t)(last_mask) * opa) >> 8);
                ...
                lv_gpu_swm330_dma2d_blend_mask(disp_buf_first, disp_w, blend_buf, draw_area_w, (const lv_opa_t *)alpha_buf, draw_area_w, draw_area_h - y);
            }
...
#elif LV_USE_GPU_SWM341_DMA2D
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_gpu_swm341_dma2d_copy(disp_buf_first, disp_w, map_buf_first, map_w, draw_area_w, draw_area_h);
                return;
            }
#elif LV_USE_GPU_SWM330_DMA2D
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_gpu_swm330_dma2d_copy(disp_buf_first, disp_w, map_buf_first, map_w, draw_area_w, draw_area_h);
                return;
            }
#endif
...
#elif LV_USE_GPU_SWM341_DMA2D
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_gpu_swm341_dma2d_blend(disp_buf_first, disp_w, map_buf_first, opa, map_w, draw_area_w, draw_area_h);
                return;
            }
#elif LV_USE_GPU_SWM330_DMA2D && !(LV_COLOR_SCREEN_TRANSP && LV_COLOR_DEPTH != 32)
            if(lv_area_get_size(draw_area) >= GPU_SIZE_LIMIT) {
                lv_gpu_swm330_dma2d_blend(disp_buf_first, disp_w, map_buf_first, map_w, opa, draw_area_w, draw_area_h);
                return;
            }
#endif
...
#if LV_USE_GPU_SWM330_DMA2D && !(LV_COLOR_SCREEN_TRANSP && LV_COLOR_DEPTH != 32)
        if(lv_area_get_size(draw_area) > GPU_SIZE_LIMIT) {
            lv_coord_t line_h = LV_HOR_RES_MAX / map_w;
            if (line_h > draw_area_h) {
                line_h = draw_area_h;
            }
            /* DMA2D extern alpha blend mode required: map_w == mask_w */
            for(y = 0; y <= draw_area_h - line_h; y += line_h) {
                for (lv_coord_t i = 0; i < line_h; ++i) {
                    for (lv_coord_t x = 0; x < draw_area_w; ++x) {
                        lv_opa_t last_mask = mask[i * draw_area_w + x];
                        alpha_buf[i * map_w + x] = (uint32_t)((uint32_t)(last_mask) * opa) >> 8;
                        //alpha_buf[i * map_w + x] = (opa > LV_OPA_MAX) ? last_mask : (last_mask >= LV_OPA_MAX ? opa : (uint32_t)((uint32_t)(last_mask) * opa) >> 8);
                    }
                }
                lv_gpu_swm330_dma2d_blend_mask(disp_buf_first, disp_w, map_buf_first, map_w, (const lv_opa_t *)alpha_buf, draw_area_w, line_h);

                disp_buf_first += disp_w * line_h;
                map_buf_first += map_w * line_h;
                mask += draw_area_w * line_h;
            }
            ...
            if(y != draw_area_h) {
                ...
                lv_gpu_swm330_dma2d_blend_mask(disp_buf_first, disp_w, map_buf_first, map_w, (const lv_opa_t *)alpha_buf, draw_area_w, draw_area_h - y);
            }
