...
void lv_draw_img(const lv_area_t * coords, const lv_area_t * mask, const void * src, const lv_draw_img_dsc_t * dsc)
{
    if(src == NULL) {
        ...
        show_error(coords, mask, "No image");
        return;
    }
...
#elif LV_USE_GPU_SWM341_DMA2D && LV_COLOR_DEPTH == 32
            /*Blend ARGB images directly*/
            if(lv_area_get_size(&draw_area) > GPU_SIZE_LIMIT) {
                int32_t disp_w = lv_area_get_width(disp_area);
                lv_color_t * disp_buf = vdb->buf_act;
                lv_color_t * disp_buf_first = disp_buf + disp_w * draw_area.y1 + draw_area.x1;
                lv_gpu_swm341_dma2d_blend(disp_buf_first, disp_w, (const lv_color_t *)map_buf_tmp, draw_dsc->opa, map_w, draw_area_w,
                                         draw_area_h);
                return;
            }
#elif LV_USE_GPU_SWM330_DMA2D && LV_COLOR_DEPTH == 32
            /*Blend ARGB images directly*/
            if(lv_area_get_size(&draw_area) > GPU_SIZE_LIMIT) {
                int32_t disp_w = lv_area_get_width(disp_area);
                lv_color_t * disp_buf = vdb->buf_act;
                lv_color_t * disp_buf_first = disp_buf + disp_w * draw_area.y1 + draw_area.x1;
                lv_gpu_swm330_dma2d_blend(disp_buf_first, disp_w, (const lv_color_t *)map_buf_tmp, map_w, draw_dsc->opa, draw_area_w,
                                         draw_area_h);
                return;
            }
#endif
