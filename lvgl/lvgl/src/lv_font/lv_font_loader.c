...
static bool lvgl_load_font(lv_fs_file_t * fp, uint32_t base_offset, lv_font_t * font);
int32_t load_kern(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint8_t format, uint32_t base_offset, uint32_t start);
...
lv_font_t * lv_font_load(const char * font_name)
{
    ...
    if(res == LV_FS_RES_OK) {
        font = lv_font_load_from_opened_file(&file);
        lv_fs_close(&file);
    }
...
lv_font_t* lv_font_load_from_opened_file(lv_fs_file_t* file)
{
    lv_font_t* font = lv_mem_alloc(sizeof(lv_font_t));
    memset(font, 0, sizeof(lv_font_t));
    uint32_t base_offset = 0;

    lv_fs_tell(file, &base_offset);
    bool success = lvgl_load_font(file, base_offset, font);

    if (!success) {
        LV_LOG_WARN("Error loading font from a opened file.\n");
        /*
...
static int read_label(lv_fs_file_t * fp, uint32_t base_offset, int start, const char * label)
{
    lv_fs_seek(fp, base_offset + start, LV_FS_SEEK_SET);
...
static bool load_cmaps_tables(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc,
                              uint32_t base_offset, uint32_t cmaps_start, 
                              cmap_table_bin_t * cmap_table)
{
    ...
    for(unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
        lv_fs_res_t res = lv_fs_seek(fp, base_offset + cmaps_start + cmap_table[i].data_offset, LV_FS_SEEK_SET);
        if(res != LV_FS_RES_OK) {
            return false;
        }
...
static int32_t load_cmaps(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint32_t base_offset, uint32_t cmaps_start)
{
    int32_t cmaps_length = read_label(fp, base_offset, cmaps_start, "cmap");
    if(cmaps_length < 0) {
        return -1;
    }
    ...
    bool success = load_cmaps_tables(fp, font_dsc, base_offset, cmaps_start, cmaps_tables);
...
static int32_t load_glyph(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc,
                          uint32_t base_offset, uint32_t start, 
                          uint32_t * glyph_offset, uint32_t loca_count, font_header_bin_t * header)
{
    int32_t glyph_length = read_label(fp, base_offset, start, "glyf");
    if(glyph_length < 0) {
        return -1;
    }
    ...
    for(unsigned int i = 0; i < loca_count; ++i) {
        ...
        lv_fs_res_t res = lv_fs_seek(fp, base_offset + start + glyph_offset[i], LV_FS_SEEK_SET);
        if(res != LV_FS_RES_OK) {
            return -1;
        }
...
static bool lvgl_load_font(lv_fs_file_t * fp, uint32_t base_offset, lv_font_t * font)
{
    ...
    /* header */
    int32_t header_length = read_label(fp, base_offset, 0, "head");
    if(header_length < 0) {
        return false;
    }
    ...
    int32_t cmaps_length = load_cmaps(fp, font_dsc, base_offset, cmaps_start);
    if(cmaps_length < 0) {
        return false;
    }
    ...
    int32_t loca_length = read_label(fp, base_offset, loca_start, "loca");
    if(loca_length < 0) {
        return false;
    }
    ...
    int32_t glyph_length = load_glyph(
                               fp, font_dsc, base_offset, glyph_start, glyph_offset, loca_count, &font_header);
    ...
    int32_t kern_length = load_kern(fp, font_dsc, font_header.glyph_id_format, base_offset, kern_start);
...
int32_t load_kern(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint8_t format, uint32_t base_offset, uint32_t start)
{
    int32_t kern_length = read_label(fp, base_offset, start, "kern");
    if(kern_length < 0) {
        return -1;
    }
