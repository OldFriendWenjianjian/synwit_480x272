...
void _lv_disp_refr_task(lv_task_t * task)
{
    ...
    if(disp_refr->inv_p != 0) {
...
#elif LV_USE_GPU_SWM341_DMA2D
                        lv_gpu_swm341_dma2d_copy((lv_color_t *)(buf_act + start_offs), disp_refr->driver.hor_res,
                                                (lv_color_t *)(buf_ina + start_offs), disp_refr->driver.hor_res,
                                                lv_area_get_width(&disp_refr->inv_areas[a]),
                                                lv_area_get_height(&disp_refr->inv_areas[a]));
#elif LV_USE_GPU_SWM330_DMA2D
                        lv_gpu_swm330_dma2d_copy((lv_color_t *)(buf_act + start_offs), disp_refr->driver.hor_res,
                                                (lv_color_t *)(buf_ina + start_offs), disp_refr->driver.hor_res,
                                                lv_area_get_width(&disp_refr->inv_areas[a]),
                                                lv_area_get_height(&disp_refr->inv_areas[a]));
#else
        ...
        _lv_memset_00(disp_refr->inv_area_nosync, sizeof(disp_refr->inv_area_nosync));
        disp_refr->inv_p = 0;
    ...
    for(join_in = 0; join_in < disp_refr->inv_p; join_in++) {
        ...
        for(join_from = 0; join_from < disp_refr->inv_p; join_from++) {
            ...
            if(lv_area_get_size(&joined_area) < (lv_area_get_size(&disp_refr->inv_areas[join_in]) +
                ...
                disp_refr->inv_area_nosync[join_in] = 0;
            }
        }
    }
}
...
static void lv_refr_areas(void)
{
    ...
    int32_t i;
    int32_t last_i = 0;
    for(i = (int32_t)disp_refr->inv_p - 1; i >= 0; i--) {
...
static void lv_refr_vdb_rotate(lv_area_t * area, lv_color_t * color_p)
{
    ...
    if(drv->rotated == LV_DISP_ROT_180) {
        ...
        uint32_t disp_rot_max_row = area_w * area_h * sizeof(lv_color_t);
        do {
            rot_buf = _lv_mem_buf_get(disp_rot_max_row);
            if (rot_buf == NULL) {
                disp_rot_max_row /= 2;
            }
        } while (rot_buf == NULL && disp_rot_max_row > 0);

        /*Determine the maximum number of rows that can be rotated at a time*/
        lv_coord_t max_row = LV_MATH_MIN((lv_coord_t)((disp_rot_max_row / sizeof(lv_color_t)) / area_w), area_h);
