...
#if LV_USE_GPU_SWM341_DMA2D
    /*Initialize DMA2D GPU*/
    lv_gpu_swm341_dma2d_init();
#endif

#if LV_USE_GPU_SWM330_DMA2D
    /*Initialize DMA2D GPU*/
    lv_gpu_swm330_dma2d_init();
#endif
...
lv_obj_t * lv_obj_create(lv_obj_t * parent, const lv_obj_t * copy)
{
    ...
    if(parent == NULL) {
        ...
        new_obj->event_cb_interceptor = NULL;
    ...
    if(copy != NULL) {
        ...
        new_obj->event_cb_data = copy->event_cb_data;
        new_obj->event_cb_interceptor = copy->event_cb_interceptor;
...
void lv_obj_set_gesture_parent_dir(lv_obj_t* obj, lv_gesture_dir_t gesture_dir)
{
    obj->gesture_parent_dir = gesture_dir;
}
...
void lv_obj_set_event_cb(lv_obj_t * obj, lv_event_cb_t event_cb)
{
    ...
    obj->event_cb_data = NULL;
}

void lv_obj_set_event_cb_with_data(lv_obj_t* obj, lv_event_cb_t event_cb, void* cb_data)
{
    LV_ASSERT_OBJ(obj, LV_OBJX_NAME);

    obj->event_cb = event_cb;
    obj->event_cb_data = cb_data;
}

void* lv_obj_get_event_cb_data(lv_obj_t* obj)
{
    LV_ASSERT_OBJ(obj, LV_OBJX_NAME);
    return obj->event_cb_data;
}

void _lv_obj_set_event_cb_interceptor(lv_obj_t* obj, lv_event_cb_interceptor_t interceptor, void *data)
{
    LV_ASSERT_OBJ(obj, LV_OBJX_NAME);
    obj->event_cb_interceptor = interceptor;
    obj->event_cb_interceptor_data = data;
}
...
lv_res_t lv_event_send_func(lv_event_cb_t event_xcb, lv_obj_t * obj, lv_event_t event, const void * data)
{
    ...
    if (obj->event_cb_interceptor) {
        obj->event_cb_interceptor(obj, event, obj->event_cb_interceptor_data, event_xcb);
    }
    else {
...
{
    if (obj->gesture_parent) {
        return true;
    }

    if (obj->gesture_parent_dir != 0) {
        lv_indev_t* indev_act = lv_indev_get_act();
        if (indev_act) {
            if (obj->gesture_parent_dir & indev_act->proc.types.pointer.gesture_dir) {
                return true;
            }
        }
    }
...
void* lv_obj_get_custom_view_data(const lv_obj_t* obj)
{
    LV_ASSERT_OBJ(obj, LV_OBJX_NAME);
    return obj->custom_view_data;
}

void lv_obj_set_custom_view_data(lv_obj_t* obj, void* data)
{
    LV_ASSERT_OBJ(obj, LV_OBJX_NAME);

    obj->custom_view_data = data;
}
...
lv_coord_t lv_obj_get_draw_rect_ext_pad_size(lv_obj_t * obj, uint8_t part)
{
    ...
    if(value_str) {
        ...
        if(value_opa > LV_OPA_MIN) {
            ...
            lv_point_t p_align;
            _lv_area_align(&obj->coords, &value_area, (lv_align_t)align, &p_align);
...
static void trans_anim_cb(lv_style_trans_t * tr, lv_anim_value_t v)
{
    ...
    if((tr->prop & 0xF) < LV_STYLE_ID_COLOR) { /*Value*/
        ...
        else x = lv_color_mix(tr->end_value._color, tr->start_value._color, (uint8_t)v);
...
static void opa_scale_anim(lv_obj_t * obj, lv_anim_value_t v)
{
    lv_obj_set_style_local_opa_scale(obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, (lv_opa_t)v);
}
