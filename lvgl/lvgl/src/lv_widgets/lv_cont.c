...
static void lv_cont_layout_center_row(lv_obj_t* cont);
...
static void lv_cont_refr_layout(lv_obj_t * cont)
{
    ...
    else if (type == LV_LAYOUT_CENTER_ROW)
    {
        lv_cont_layout_center_row(cont);
    }
    else if(type == LV_LAYOUT_COLUMN_LEFT || type == LV_LAYOUT_COLUMN_MID || type == LV_LAYOUT_COLUMN_RIGHT || type == LV_LAYOUT_COLUMN_BOTTOM_MID) {
...
static void lv_cont_layout_col(lv_obj_t * cont)
{
    ...
    switch(type) {
        ...
        case LV_LAYOUT_COLUMN_RIGHT:
            ...
            break;
        case LV_LAYOUT_COLUMN_BOTTOM_MID:
            hpad_corr = 0;
            align = LV_ALIGN_IN_BOTTOM_MID;
            break;
        default:
            hpad_corr = 0;
            align     = LV_ALIGN_IN_TOP_LEFT;
    ...
    if (align == LV_ALIGN_IN_BOTTOM_MID) {
        last_cord = lv_obj_get_style_pad_bottom(cont, LV_CONT_PART_MAIN);;
    }
    else {
        last_cord = lv_obj_get_style_pad_top(cont, LV_CONT_PART_MAIN);
    }
...
static void lv_cont_layout_center_row(lv_obj_t* cont)
{
    ...
    _LV_LL_READ(cont->child_ll, child) {
        if (lv_obj_get_hidden(child) != false || lv_obj_is_protected(child, LV_PROTECT_POS) != false) continue;
        w_tot += lv_obj_get_width(child) + inner;
        obj_num++;
    }
    ...
    /* Align the children */
    lv_coord_t last_cord = -(w_tot / 2);
    _LV_LL_READ_BACK(cont->child_ll, child) {
        if (lv_obj_get_hidden(child) != false || lv_obj_is_protected(child, LV_PROTECT_POS) != false) continue;

        lv_obj_align(child, cont, LV_ALIGN_CENTER, last_cord + lv_obj_get_width(child) / 2, 0);
        last_cord += lv_obj_get_width(child) + inner;
    }
