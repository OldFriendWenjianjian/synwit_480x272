...
static lv_design_res_t lv_chart_design(lv_obj_t * chart, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        lv_chart_ext_t* ext = lv_obj_get_ext_attr(chart);
        lv_chart_axis_cfg_t* x_axis = &ext->x_axis;
        lv_chart_axis_cfg_t* y_axis = &ext->y_axis;
        uint8_t iter_dir;
        lv_chart_label_iterator_t iter;
        uint8_t num_of_labels;
        uint8_t i;
        char buf[LV_CHART_AXIS_TICK_LABEL_MAX_LEN + 1];

        lv_draw_label_dsc_t label_dsc;
        lv_draw_label_dsc_init(&label_dsc);
        lv_obj_init_draw_label_dsc(chart, LV_CHART_PART_BG, &label_dsc);
        lv_style_int_t label_dist = lv_obj_get_style_pad_left(chart, LV_CHART_PART_SERIES_BG);

        lv_point_t max_size = { 0,0 }, temp_size = { 0,0 };
        //纵坐标处理
        if (y_axis->list_of_values != NULL)
        {
            ...
            for (i = 0; i < (num_of_labels + 1); i++) {
                ...
                _lv_txt_get_size(&temp_size, buf, label_dsc.font, label_dsc.letter_space, label_dsc.line_space,
                    LV_COORD_MAX, LV_TXT_FLAG_CENTER);
                if (temp_size.x > max_size.x)
                {
                    max_size.x = temp_size.x;
                }
            }

            if (strlen(y_axis->list_of_values) == 0)
            {
                lv_obj_set_style_local_pad_left(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, 5);
                lv_obj_set_style_local_pad_top(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, 5);
                lv_obj_set_style_local_pad_bottom(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, 5);
            }
            else
            {
                lv_obj_set_style_local_pad_left(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, label_dist + max_size.x + 5);
                lv_obj_set_style_local_pad_top(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, label_dsc.font->line_height / 2);
                lv_obj_set_style_local_pad_bottom(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, label_dsc.font->line_height / 2);
            }
        }
        else
        {
        ...
        //横坐标处理
        if (x_axis->list_of_values != NULL)
        {
            iter = create_axis_label_iter(x_axis->list_of_values, LV_CHART_LABEL_ITERATOR_FORWARD);
            num_of_labels = iter.items_left;
            ...
            if (strlen(x_axis->list_of_values) == 0)
            {
                if (label_dsc.font->line_height / 2 < 5)
                {
                    lv_obj_set_style_local_pad_bottom(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, 5);
                }
            }
            ...
            {
                lv_obj_set_style_local_pad_bottom(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, label_dist + label_dsc.font->line_height + 5);
            }
            lv_obj_set_style_local_pad_right(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, temp_size.x / 2);
            if (y_axis->list_of_values == NULL)
            {
                iter = create_axis_label_iter(x_axis->list_of_values, LV_CHART_LABEL_ITERATOR_FORWARD);
                get_next_axis_label(&iter, buf);
                _lv_txt_get_size(&temp_size, buf, label_dsc.font, label_dsc.letter_space, label_dsc.line_space,
                    LV_COORD_MAX, LV_TXT_FLAG_CENTER);
                lv_obj_set_style_local_pad_left(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, temp_size.x / 2);
            }

        }
        else
        {
            lv_obj_set_style_local_pad_right(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, 5);
        }
...
static void draw_series_line(lv_obj_t * chart, const lv_area_t * series_area, const lv_area_t * clip_area)
{
    ...
    if(has_area) {
        ...
        if(has_fade) {
            lv_draw_mask_fade_init(&mask_fade_p, series_area, (lv_opa_t)area_dsc.bg_main_color_stop, series_area->y1,
                (lv_opa_t)area_dsc.bg_grad_color_stop, series_area->y2);
        }
    }
