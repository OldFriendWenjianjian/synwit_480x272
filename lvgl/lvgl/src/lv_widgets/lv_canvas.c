...
void lv_canvas_blur_hor(lv_obj_t * canvas, const lv_area_t * area, uint16_t r)
{
    ...
    if(area) {
        ...
        if(a.x1 < 0) a.x1 = 0;
        if(a.y1 < 0) a.y1 = 0;
        if(a.x2 > (lv_coord_t)ext->dsc.header.w - 1) a.x2 = ext->dsc.header.w - 1;
        if(a.y2 > (lv_coord_t)ext->dsc.header.h - 1) a.y2 = ext->dsc.header.h - 1;
    }
    else {
        a.x1 = 0;
        a.y1 = 0;
    ...
    for(y = a.y1; y <= a.y2; y++) {
        ...
        for(x = a.x1 - r_back; x <= a.x1 + r_front; x++) {
            x_safe = x < 0 ? 0 : x;
            x_safe = x_safe > (lv_coord_t)ext->dsc.header.w - 1 ? (lv_coord_t)ext->dsc.header.w - 1 : x_safe;
...
void lv_canvas_blur_ver(lv_obj_t * canvas, const lv_area_t * area, uint16_t r)
{
    ...
    if(area) {
        ...
        if(a.x1 < 0) a.x1 = 0;
        if(a.y1 < 0) a.y1 = 0;
        if(a.x2 > (lv_coord_t)ext->dsc.header.w - 1) a.x2 = (lv_coord_t)ext->dsc.header.w - 1;
        if(a.y2 > (lv_coord_t)ext->dsc.header.h - 1) a.y2 = (lv_coord_t)ext->dsc.header.h - 1;
    }
    else {
        a.x1 = 0;
        a.y1 = 0;
        a.x2 = (lv_coord_t)ext->dsc.header.w - 1;
        a.y2 = (lv_coord_t)ext->dsc.header.h - 1;
    }
    ...
    for(x = a.x1; x <= a.x2; x++) {
        ...
        for(y = a.y1 - r_back; y <= a.y1 + r_front; y++) {
            y_safe = y < 0 ? 0 : y;
            y_safe = y_safe > (lv_coord_t)ext->dsc.header.h - 1 ? (lv_coord_t)ext->dsc.header.h - 1 : y_safe;
