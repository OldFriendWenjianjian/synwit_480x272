...
lv_obj_t * lv_imgbtn_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    _lv_memset_00((void*)ext->recolor, sizeof(ext->recolor));
    _lv_memset_00((void*)ext->recolor_opa, sizeof(ext->recolor_opa));
#if LV_IMGBTN_TILED
    ...
    /*Copy an existing image button*/
        ...
        _lv_memcpy((void*)ext->recolor, copy_ext->recolor, sizeof(ext->recolor));
        _lv_memcpy((void*)ext->recolor_opa, copy_ext->recolor_opa, sizeof(ext->recolor_opa));
#if LV_IMGBTN_TILED
...
void lv_imgbtn_set_recolor(lv_obj_t* imgbtn, lv_btn_state_t state, lv_color_t recolor)
{
    LV_ASSERT_OBJ(imgbtn, LV_OBJX_NAME);

    lv_imgbtn_ext_t* ext = lv_obj_get_ext_attr(imgbtn);
    ext->recolor[state] = recolor;
}

void lv_imgbtn_set_recolor_opa(lv_obj_t* imgbtn, lv_btn_state_t state, lv_opa_t recolor_opa)
{
    LV_ASSERT_OBJ(imgbtn, LV_OBJX_NAME);

    lv_imgbtn_ext_t* ext = lv_obj_get_ext_attr(imgbtn);
    ext->recolor_opa[state] = recolor_opa;
}
    ...
    /*Draw the object*/
        ...
        if(!ext->tiled) {
            ...
            if(lv_img_src_get_type(src) == LV_IMG_SRC_SYMBOL) {
                ...
                /*Overwrite recolor & recolor_opa*/
                lv_btn_state_t ori_state = lv_imgbtn_get_state(imgbtn);
                img_dsc.recolor_opa = ext->recolor_opa[ori_state];
                if (img_dsc.recolor_opa > 0) {
                    img_dsc.recolor = ext->recolor[ori_state];
                }
...
static void refr_img(lv_obj_t * imgbtn)
{
    ...
    if(info_res == LV_RES_OK) {
        ...
        ext->img_w = header.w;
        ext->img_h = header.h;
        if (lv_obj_get_width(imgbtn) == 0 && lv_obj_get_height(imgbtn) == 0) {
