...
static lv_fs_res_t fs_seek (lv_fs_drv_t * drv, void * file_p, int32_t pos,lv_fs_whence_t whence);
...
void lv_fs_if_pc_init(void)
{
	...
	char cur_path[512];
	_getcwd(cur_path, sizeof(cur_path));
 ...
 *   STATIC FUNCTIONS
 **********************/
static int utf8_to_local(const char* path, char* out_local, int out_local_size)
{
	int wideLen = MultiByteToWideChar(CP_UTF8, 0, path, -1, NULL, 0);
	wchar_t* pwBuf = malloc((wideLen + 1) * sizeof(wchar_t));
	if (pwBuf == NULL) {
		return 0;
	}
	memset(pwBuf, 0, (wideLen + 1) * sizeof(wchar_t));
	MultiByteToWideChar(CP_UTF8, 0, path, strlen(path), pwBuf, wideLen);

	int charLen = WideCharToMultiByte(CP_ACP, 0, pwBuf, -1, NULL, 0, NULL, NULL);
	char* pBuf = malloc(charLen + 1);
	if (pBuf == NULL) {
		free(pwBuf);
		return 0;
	}
	memset(pBuf, 0, charLen + 1);
	WideCharToMultiByte(CP_ACP, 0, pwBuf, wideLen, pBuf, charLen, NULL, NULL);

	snprintf(out_local, out_local_size, "%s", pBuf);
	free(pwBuf);
	free(pBuf);
...
static lv_fs_res_t fs_open (lv_fs_drv_t * drv, void * file_p, const char * path, lv_fs_mode_t mode)
{
	...
	char local_encoding_path[2048];
	char buf[2048];
	sprintf(buf, LV_FS_PC_PATH "%s", path);
	utf8_to_local(buf, local_encoding_path, sizeof(local_encoding_path));

	file_t f = fopen(local_encoding_path, flags);
 ...
 * @param whence the start position of read write pointer
 * @return LV_FS_RES_OK: no error, the file is read
 *         any error from lv_fs_res_t enum
 */
static lv_fs_res_t fs_seek (lv_fs_drv_t * drv, void * file_p, int32_t pos, lv_fs_whence_t whence)
...
static lv_fs_res_t fs_dir_read (lv_fs_drv_t * drv, void * dir_p, char *fn)
{
	...
	if(FindNextFileA(*dp, &fdata) == false) return LV_FS_RES_OK;
	do {
		if (strcmp(fdata.cFileName, ".") == 0 || strcmp(fdata.cFileName, "..") == 0) {
			continue;
		} else {
